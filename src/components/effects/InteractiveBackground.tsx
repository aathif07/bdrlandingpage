  ...
  useEffect(() => {
    ...
    class Particle {
      ...
      draw() {
        ...
        // Set color based on theme - using grayish-brown (#8B8378) for dark theme
        if (this.isDark) {
          ctx.fillStyle = 'rgba(139, 131, 120, 0.6)'; // Grayish-brown with opacity
        } else {
          ctx.fillStyle = 'rgba(166, 159, 147, 0.6)'; // Lighter grayish-brown for light theme
        }
      ...
      for (let a = 0; a < particlesArray.length; a++) {
        for (let b = a; b < particlesArray.length; b++) {
          ...
          if (distance < 120) { // Increased connection distance
            // Set line color based on theme - using grayish-brown variations
            if (theme === 'dark') {
              ctx.strokeStyle = `rgba(139, 131, 120, ${0.3 - distance/400})`; // Darker grayish-brown
            } else {
              ctx.strokeStyle = `rgba(166, 159, 147, ${0.3 - distance/400})`; // Lighter grayish-brown
            }
            ctx.lineWidth = 1;
            ctx.beginPath();
